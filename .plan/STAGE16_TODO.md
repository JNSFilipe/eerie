...
Eerie-native commands for `f` and `w` so the external file is no longer
needed at runtime.

## Scope

- [x] Add a Eerie-owned visible jump loop for current-window candidates
...
- [x] Reuse Eerie overlays and jumplist helpers instead of depending on
  `avy.el`
