...
- Stage 43 classified `eerie-find-ref`, clipboard, comment, page,
  slurp, sexp, wrap, open, and replace helpers as keep for now.
- `eerie-visual-search-next-or-multicursor` is not a Stage 45 target;
...
- Stage 45 may still remove stale references from `eerie-var.el`,
  `eerie-beacon.el`, and `eerie-tutor.el`, but only for helpers this pass
  confirms are dead.

## Tasks

- [x] Remove any helpers this pass proves unreachable, excluding
  `eerie-visual-search-next-or-multicursor`
- [x] Remove the matching stale references from `eerie-var.el`,
  `eerie-beacon.el`, and `eerie-tutor.el` for the helpers actually
...
- Removed the unreachable `eerie-open-above`, `eerie-open-above-visual`,
  `eerie-open-below`, and `eerie-open-below-visual` command family from
  `eerie-command.el`.
- Removed the dead `eerie-select-on-open` custom, the stale
  `eerie-open-above` and `eerie-open-below` indicator labels, and the
