...
;; - `C-n`, `C-p`, `C-b`, `C-f`: from normal mode, they should move like regular Emacs while `h j k l` still work. On long wrapped lines, `j`, `k`, `C-n`, and `C-p` should follow wrapped screen lines instead of jumping straight to the next logical line.
;; - `C-x` and `C-c`: from normal mode, they should open the regular Emacs prefix maps, while `SPC` keeps opening Eerie's own leader menu.
