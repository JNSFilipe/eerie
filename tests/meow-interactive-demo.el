...
;; - `W`: from the beginning of `targetword` or the operator lines below, move to the next space on the current line; repeated `W` should continue to the following space, and once no later space remains it should fall back to the end of the line.
...
;; - `m`: select one `targetword` with `v` or `w`, then press `m`, `m`, `,`, `;`, `m`, `s`, `m`, and `ESC`. The original selected text should stay frozen as the match seed, older matches should remain highlighted as secondary overlays, `,` should remove the newest target, `s` should skip one hidden candidate without selecting it, and `ESC` should clear the whole multi-edit session.
;; - Multi-edit actions: on the `targetword` line below, select one `targetword`, grow with `m`, then try `i` and `a` with some inserted text before `ESC`, and then repeat with `d` and `c`. Multi-edit `i` should insert at the beginning of every selected target, multi-edit `a` should append after every selected target, `d` should delete them all, and `c` should change them all to the same inserted text.
;; - Multi-cursor: after growing a multi-edit selection with `m`, press `n` to promote it into multi-cursor normal mode. Then try normal commands like `h`, `l`, `x`, `%`, `gg`, or `G`, plus multi-cursor `f<char>` and `W` on the parallel lines below. Insert-like commands such as `i`, `a`, `I`, `A`, or `c` should still replay on `ESC`. `ESC` from the multi-cursor state itself should cancel the full cursor set.
