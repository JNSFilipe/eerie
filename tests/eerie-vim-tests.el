...
(ert-deftest eerie-default-normal-keymap-is-vim-like ()
  ...
  (should (eq (lookup-key eerie-normal-state-keymap (kbd "C-b")) 'eerie-left))
  (should (eq (lookup-key eerie-normal-state-keymap (kbd "C-f")) 'eerie-right))
  (should (eq (lookup-key eerie-normal-state-keymap (kbd "C-n")) 'eerie-next))
  (should (eq (lookup-key eerie-normal-state-keymap (kbd "C-p")) 'eerie-prev))
  ...
  (should (eq (lookup-key eerie-normal-state-keymap (kbd "p")) 'eerie-yank))
  (should (eq (lookup-key eerie-normal-state-keymap (kbd "C-x")) ctl-x-map))
  (should (eq (lookup-key eerie-normal-state-keymap (kbd "C-c")) mode-specific-map))
  ...
  (should (eq (lookup-key eerie-multicursor-state-keymap (kbd "f"))
              'eerie-jump-char))
  (should (eq (lookup-key eerie-multicursor-state-keymap (kbd "C-f"))
              'eerie-right))
  (should (eq (lookup-key eerie-multicursor-state-keymap (kbd "W"))
              'eerie-next-space))
  (should (eq (lookup-key eerie-multicursor-state-keymap (kbd "C-n"))
              'eerie-next))
  (should (eq (lookup-key eerie-multicursor-state-keymap (kbd "C-p"))
              'eerie-prev))
  (should (eq (lookup-key eerie-multicursor-state-keymap (kbd "C-b"))
              'eerie-left))
  (should (eq (lookup-key eerie-multicursor-state-keymap (kbd "n"))
              'eerie-search-next))
  (should (eq (lookup-key eerie-multicursor-state-keymap (kbd "p"))
              'eerie-yank))
...
(ert-deftest eerie-normal-c-c-preserves-major-mode-prefix-bindings ()
  (let ((before-eval nil)
        (before-compile nil)
        (after-eval nil)
        (after-compile nil))
    (with-temp-buffer
      (emacs-lisp-mode)
      (setq before-eval (key-binding (kbd "C-c C-e")))
      (setq before-compile (key-binding (kbd "C-c C-b")))
      (eerie-mode 1)
      (setq after-eval (key-binding (kbd "C-c C-e")))
      (setq after-compile (key-binding (kbd "C-c C-b"))))
    (should (eq before-eval 'elisp-eval-region-or-buffer))
    (should (eq before-compile 'elisp-byte-compile-buffer))
    (should (eq after-eval before-eval))
    (should (eq after-compile before-compile))))

(ert-deftest eerie-normal-motion-keys-and-control-aliases-move-point ()
...
(ert-deftest eerie-normal-vertical-motions-follow-wrapped-screen-lines ()
  (eerie-test-with-buffer "abcdefghijklmnopqrstuvwxyz\n"
    (let ((right (split-window-right)))
      (window-resize right (- 12 (window-total-width right)) t)
      (select-window right))
    (setq truncate-lines nil)
    (visual-line-mode 1)
    (let ((expected-next (save-excursion
                           (vertical-motion 1)
                           (point))))
      (should (> (count-screen-lines (point-min) (point-max)) 1))
      (execute-kbd-macro (kbd "j"))
      (should (= (point) expected-next))
      (should (= (line-number-at-pos) 1))
      (execute-kbd-macro (kbd "k"))
...
(ert-deftest eerie-normal-vertical-motions-step-cleanly-through-wrapped-lines ()
  (eerie-test-with-buffer "abcdefghijklmnopqrstuvwxyz\ncd\n"
    (let ((right (split-window-right)))
      (window-resize right (- 12 (window-total-width right)) t)
      (select-window right))
    (setq truncate-lines nil)
    (visual-line-mode 1)
    (should (> (count-screen-lines (point-min) (point-max)) 3))
    (execute-kbd-macro (kbd "j"))
    (should (= (point) 12))
    (execute-kbd-macro (kbd "j"))
    (should (= (point) 23))
...
(ert-deftest eerie-leader-map-is-separate-from-vanilla-c-c-prefix ()
  (let ((leader-map (alist-get 'leader eerie-keymap-alist)))
    (should (keymapp leader-map))
    (should-not (eq leader-map mode-specific-map))
    (should (eq (lookup-key leader-map (kbd "1")) 'eerie-digit-argument))
    (should (eq (lookup-key leader-map (kbd "?")) 'eerie-cheatsheet))
    (should-not (eq (lookup-key mode-specific-map (kbd "1"))
                    'eerie-digit-argument))
